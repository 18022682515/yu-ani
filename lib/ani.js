...
function isTarget(o1,o2){	//判断是否已达到目标值
	let bool = false;
	
	if(getType(o1,'Object')){
		let key = Object.keys(o1).find(key=>getType(o1[key],'Number'));
		if(key){
			bool = o1[key]>=o2[key] ?true :false;
		}else{
			bool = Object.keys(o1).some(key=>isTarget(o1[key],o2[key]))
		}
	}else if(getType(o1,'Array')){
		let i = o1.findIndex(val=>getType(val,'Number'));
		if(i!==-1){
			bool = o1[i]>=o2[i] ?true :false;
		}else{
			bool = o1.some((val,i)=>isTarget(val,o2[i]))
		}
	}
	return bool;
}
...
function run(initial,current,speeds,target,update,complete,status){	//启动动画
	(function fn(){
		...
		if(isTarget(current,target) || status.end){
			current = copy(target);
			getType(update,'Function') && update(current);
